...
def ErGOT(Gx, Gy, IterNum, tau, n_samples, epochs, lr, epsilon, PlotLoss=True, verbose=True):
    ...
    for epoch in range(epochs):
        ...
        if verbose and (epoch==0 or (epoch+1) % 100 == 0):
            ...
            StartTime=time.time()
    # PyTorch -> NumPy
    P = DoublyStochasticMatrix(mean, tau, IterNum)
    P = P.squeeze()
    P = P.detach().numpy()
...
def Optimization_ErGOT(x, y, IterNum, tau, n_samples, epochs, lr, epsilon, alpha = 2, ones = 1, graphs = True, PlotLoss=True, verbose=True, Trick=False):
    Gx = GraphRepresentation(x, alpha, ones,Trick)
    Gy = GraphRepresentation(y, alpha, ones,Trick)
    P, TotalTimeCost = ErGOT(Gx, Gy, IterNum, tau, n_samples, epochs, lr, epsilon, PlotLoss=True, verbose=True)
